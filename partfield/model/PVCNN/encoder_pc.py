...
class TriPlanePC2Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.use_point_scatter = use_point_scatter

        self.z_triplane_resolution = z_triplane_resolution
        self.z_triplane_channels = z_triplane_channels

        point_encoder_out_dim = z_triplane_channels #* 2

        in_channels = 6
        # self.resample_filter=[1, 3, 3, 1]
        if point_encoder_type == 'pvcnn':
            self.pc_encoder = PVCNNEncoder(point_encoder_out_dim, 
            device=self.device, in_channels=in_channels, use_2d_feat=use_2d_feat,is_My=is_My)  # Encode it to a volume vector.
        elif point_encoder_type == 'pointnet':
            ...
            raise NotImplementedError(f"Point encoder {point_encoder_type} not implemented")
    ...
    def encode(self, point_cloud_xyz, point_cloud_feature, mv_feat=None, pc2pc_idx=None,add_features=None) -> Dict:
        # output = AttrDict()
        ...
        if self.point_encoder_type == 'pvcnn':
            if mv_feat is not None:
                pc_feat, points_feat = self.pc_encoder(point_cloud, mv_feat, pc2pc_idx)
            else:
                pc_feat, points_feat = self.pc_encoder(point_cloud,add_features=add_features)  # 3D feature volume: BxDx32x32x32
            if self.use_point_scatter:
        ...
        elif self.point_encoder_type == 'pointnet':
            assert self.use_point_scatter
            # Run ConvPointnet
    ...
    def forward(self, point_cloud_xyz, point_cloud_feature=None, mv_feat=None, pc2pc_idx=None,add_features=None):
        return self.encode(point_cloud_xyz, point_cloud_feature=point_cloud_feature, mv_feat=mv_feat, pc2pc_idx=pc2pc_idx,add_features=add_features)